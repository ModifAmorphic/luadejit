...
//! edges. The CFG is not yet consumed by the emit pipeline — the
//! walk-based emitter in [`crate::emit`] continues to handle
//! decompilation unchanged. This module is pure infrastructure that
//! later stages (SSA, structural recovery) will build on.
//!
...
// TODO(Stage 7b): add `pub dominator_tree: DominatorTree`. The
// architecture doc lists it on `Cfg`, but dominator-tree computation
// is deferred to Stage 7b; Stage 7a only needs blocks + edges.
#[derive(Clone, Debug)]
pub struct Cfg {
